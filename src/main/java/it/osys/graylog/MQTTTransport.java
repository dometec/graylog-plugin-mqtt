...
public class MQTTTransport implements Transport, MqttCallbackExtended, IMqttActionListener {
	...
	public void launch(MessageInput messageInput) throws MisfireException {
		...
		final String clientId = configuration.getString(CK_USE_AUTH,
				"graylog_" + Hashing.murmur3_32_fixed().hashUnencodedChars(this.serverStatus.getNodeId().toString()).toString());
		...
		while (true) {
			...
			try {
				...
				for (String topic : topics) {
					logger.info("Subscription to topic {} qos {} ...", topic, qos);
	...
	public void messageArrived(String topic, MqttMessage message) {
		...
		// MQTTMessage m = new MQTTMessage(topic, message);
		HashMap<String, Object> m = new HashMap<>();
		m.put("payload", new String(message.getPayload(), StandardCharsets.UTF_8));
		m.put("topic", topic);
		m.put("qos", message.getQos());
		m.put("mqttmessageid", message.getId());
		m.put("duplicate", message.isDuplicate());
		m.put("retained", message.isRetained());
	...
	public static class Config implements Transport.Config {
		...
		public ConfigurationRequest getRequestedConfiguration() {
			...
			cr.addField(new NumberField(CK_QOS, "QoS Level", 0, "QoS Level you are subscribing to (0, 1, or 2)."));

			cr.addField(new TextField(CK_CLIENTID, "Client ID", "", "The ClientId of client."));
